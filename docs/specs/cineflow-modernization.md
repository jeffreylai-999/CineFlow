...
- Collapse the reason a Seat is unavailable in the customer-facing availability representation itself, not only in the interface. A customer response reports a Seat as available or unavailable and nothing more; Seat Hold, Booking, and Disabled Seat are distinguished only in responses to authorized Staff. Collapsing only in React would be bypassable through developer tools.
- Understand what this does and does not conceal. Story 14 deliberately shows customers live Seat changes, so a Seat becoming unavailable remains visible and its timing is not hidden. What is concealed is the cause: a customer cannot tell a ten-minute Seat Hold that may expire from a confirmed Booking that never will, which is the signal worth gaming. Do not state or design for a stronger guarantee than that.
...
- Run `axe-core` directly inside the browser-mode test against every route, with dialogs and menus opened first, because axe does not inspect closed or hidden regions. Do not use `@axe-core/playwright`: its `AxeBuilder` requires a Playwright `Page`, which Vitest's browser context does not expose.
- Treat automated accessibility checks as a floor, not the target. Automated tooling detects no Focus Order, Focus Visible, Non-text Contrast, or Meaningful Sequence issues, and no library in the interface stack claims WCAG conformance.
- Run two manual passes over catalog browsing, Seat selection, checkout, administration, and Admission each release. A keyboard-only walkthrough covers Focus Order and Focus Visible. A reading-order pass, by screen reader or by reading the DOM against the rendered layout, covers Meaningful Sequence, which a keyboard walkthrough cannot reach because it only visits focusable controls. Give particular attention to the docked Booking summary and the Seat Map, where visual order and DOM order can diverge.
- Measure Seat-state contrast ratios by hand and record them beside the token definitions rather than assuming they pass.
