...
Vendored components sit on Base UI, which became the shadcn default in July 2026. Radix is not deprecated and almost every component ships for both, so `shadcn init -b radix` remains a usable escape hatch — with one exception CineFlow creates for itself. `toast` exists only in the Base UI registry, so switching to Radix would mean replacing it, and the only registry alternative is `sonner`, which brings the `next-themes` dependency this project rejected. Treat the notification surface as the single component a base switch would have to rewrite. Two component choices exist specifically to keep the primitive tree single-sourced: `combobox` is used for provider search instead of `command`, because `command` depends on `cmdk` and therefore installs Radix packages into a Base UI project, and `toast` is used instead of `sonner`, which pulls `next-themes` into a Vite application that has no use for it.
